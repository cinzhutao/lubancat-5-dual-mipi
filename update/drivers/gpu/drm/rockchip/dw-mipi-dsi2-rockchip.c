...
#define CMD_READ_POLL_US		20
#define CMD_READ_TIMEOUT_US		800
...
static bool dw_mipi_dsi2_msg_use_lpm(struct dw_mipi_dsi2 *dsi2,
				     const struct mipi_dsi_msg *msg)
{
	return msg->flags & MIPI_DSI_MSG_USE_LPM ||
	       dsi2->mode_flags & MIPI_DSI_MODE_LPM;
}
					 ...
					 unsigned long *mode_flags)
{
	...
	dsi2 = host_to_dsi2(host);
	*mode_flags = dsi2->mode_flags;
	...
	dev_info(dsi2->dev,
		 "dsi host force_lpm=%d saved_mode_flags=0x%lx active_mode_flags=0x%lx\n",
		 force_lpm, *mode_flags, dsi2->mode_flags);

	return 0;
}
EXPORT_SYMBOL_GPL(rockchip_dw_mipi_dsi2_host_force_lpm);
						   ...
						   unsigned long mode_flags)
{
	...
	dsi2 = host_to_dsi2(host);
	dsi2->mode_flags = mode_flags;
	if (dsi2->slave)
		dsi2->slave->mode_flags = mode_flags;

	dev_info(dsi2->dev, "dsi host restore mode_flags=0x%lx\n",
		 mode_flags);
}
EXPORT_SYMBOL_GPL(rockchip_dw_mipi_dsi2_host_restore_mode_flags);

static bool dw_mipi_dsi2_rx_packet_is_short(u8 data_type)
{
	switch (data_type) {
...
static int dw_mipi_dsi2_rx_short_payload_len(u8 data_type)
{
	switch (data_type) {
	case MIPI_DSI_RX_GENERIC_SHORT_READ_RESPONSE_1BYTE:
	case MIPI_DSI_RX_DCS_SHORT_READ_RESPONSE_1BYTE:
...
static bool dw_mipi_dsi2_rx_packet_is_long(u8 data_type)
{
	switch (data_type) {
...
static bool dw_mipi_dsi2_rx_matches_msg(const struct mipi_dsi_msg *msg,
					u8 data_type)
{
	switch (msg->type) {
...
static bool dw_mipi_dsi2_msg_is_standard_read(const struct mipi_dsi_msg *msg)
{
	switch (msg->type) {
	case MIPI_DSI_DCS_READ:
	case MIPI_DSI_GENERIC_READ_REQUEST_0_PARAM:
...
static void dw_mipi_dsi2_drain_rx_fifo(struct dw_mipi_dsi2 *dsi2,
				       const char *tag)
{
	u32 sts, hdr, val;
	u16 wc;
	...
	for (i = 0; i < 8; i++) {
		...
		DRM_DEV_INFO(dsi2->dev,
			     "%s: drain stale rx hdr=0x%08x type=0x%02x wc=%u\n",
			     tag, hdr, data_type, wc);

		if (!dw_mipi_dsi2_rx_packet_is_long(data_type))
			continue;

		words = DIV_ROUND_UP(wc, 4);
		while (words--)
			regmap_read(dsi2->regmap, DSI2_CRI_RX_PLD, &val);
	}

	dev_warn(dsi2->dev, "%s: rx fifo still not empty after drain\n", tag);
}
...
static void dw_mipi_dsi2_set_vid_mode(struct dw_mipi_dsi2 *dsi2)
{
	...
	if (!(dsi2->mode_flags & MIPI_DSI_MODE_LPM) &&
	    dsi2->mode_flags & MIPI_DSI_MODE_VIDEO_NO_HFP)
		val |= BLK_HFP_HS_EN;

	if (!(dsi2->mode_flags & MIPI_DSI_MODE_LPM) &&
	    dsi2->mode_flags & MIPI_DSI_MODE_VIDEO_NO_HBP)
		val |= BLK_HBP_HS_EN;
	...
	DRM_DEV_INFO(dsi2->dev,
		     "video tx cfg=0x%08x mode_flags=0x%lx lpdt_cmd=%d blank_lp=%d\n",
		     val, dsi2->mode_flags,
		     !!(val & LPDT_DISPLAY_CMD_EN),
		     !!(dsi2->mode_flags & MIPI_DSI_MODE_LPM));
				      ...
				      const struct mipi_dsi_msg *msg)
{
	...
	DRM_DEV_INFO(dsi2->dev,
		     "dsi read rx hdr=0x%08x type=0x%02x req_type=0x%02x rx_len=%zu\n",
		     hdr, data_type, msg->type, msg->rx_len);

	if (data_type == MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT) {
		DRM_DEV_ERROR(dsi2->dev,
			      "DSI read returned ACK/error report: hdr=0x%08x\n",
			      hdr);
		return -EIO;
	}

	if (dw_mipi_dsi2_msg_is_standard_read(msg) &&
	    !dw_mipi_dsi2_rx_matches_msg(msg, data_type)) {
		DRM_DEV_ERROR(dsi2->dev,
			      "invalid DSI read response type: hdr=0x%08x type=0x%02x req_type=0x%02x\n",
			      hdr, data_type, msg->type);
		return -EIO;
	}

	if (dw_mipi_dsi2_rx_packet_is_short(data_type)) {
		rx_len = min_t(u16, len,
			       dw_mipi_dsi2_rx_short_payload_len(data_type));
		for (i = 0; i < rx_len; i++)
			payload[i] = (hdr >> (8 * (i + 1))) & 0xff;

		DRM_DEV_INFO(dsi2->dev,
			     "dsi short read response: type=0x%02x rx_len=%u data=%*ph\n",
			     data_type, rx_len, rx_len, payload);
	...
	if (!dw_mipi_dsi2_rx_packet_is_long(data_type)) {
		DRM_DEV_ERROR(dsi2->dev,
			      "unsupported DSI read response type: hdr=0x%08x type=0x%02x\n",
			      hdr, data_type);
		return -EIO;
	}

	wc = (hdr >> 8) & 0xffff;
	rx_len = min_t(u16, len, wc);
	/* Receive payload */
	...
	DRM_DEV_INFO(dsi2->dev,
		     "dsi long read response: type=0x%02x wc=%u rx_len=%u data=%*ph\n",
		     data_type, wc, rx_len, rx_len, payload);
				    ...
				    const struct mipi_dsi_msg *msg)
{
	...
	pm_runtime_get_sync(dsi2->dev);
	dw_mipi_dsi2_clk_management(dsi2);
	use_lpm = dw_mipi_dsi2_msg_use_lpm(dsi2, msg);
	regmap_update_bits(dsi2->regmap, DSI2_DSI_VID_TX_CFG, LPDT_DISPLAY_CMD_EN,
			   use_lpm ? LPDT_DISPLAY_CMD_EN : 0);
	regmap_read(dsi2->regmap, DSI2_DSI_VID_TX_CFG, &vid_tx_cfg);
	...
	if (msg->rx_len)
		dw_mipi_dsi2_drain_rx_fifo(dsi2, "pre-read");
	...
	/* Send packet header */
	mode = CMD_TX_MODE(use_lpm ? 1 : 0);
	val = get_unaligned_le32(packet.header);

	dev_info(dsi2->dev,
		 "dsi transfer type=0x%02x len=%zu rx=%zu flags=0x%04x use_lpm=%d cmd_tx_mode=%d lpdt_display_cmd_en=%d mode_flags=0x%lx\n",
		 msg->type, msg->tx_len, msg->rx_len, msg->flags, use_lpm,
		 !!(mode & CMD_TX_MODE(1)),
		 !!(vid_tx_cfg & LPDT_DISPLAY_CMD_EN), dsi2->mode_flags);
	...
	return msg->rx_len ? ret : msg->tx_len;
